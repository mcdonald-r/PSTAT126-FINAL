...
outliers they are still important to note and show that even within a sample size of 500 observations the presence of 
outliers is there. 
...
better the predictions would be or if it wouldn't matter in this case. In this model, I decided to transform the y
variable as I saw it looked more interesting than transforming the x variable. For this I'm looking into the relation
between the price and how it is affected by the size of the table in relation to the diamond. First comparing the
summary statistics, just looking at the residuals you can see the difference in the two different models. In the
transformed model you can see how the median is just around 0 and the min and max values are just around -2 and 2, 
compared to the first model where the median was -1614 and the min and max were -4673 and 14772. The transformed model 
creates a median that is actually in the middle of the min and max and brings those small and large values closer
together. When it came to the coefficients, the first model didn't show any significance between the two tested 
variables, however in the transformed model, we can now see how the intercept has a significance code of *** and the 
table has a code of **. This is a huge change compared to what was seen in the first model where there was no
significance at all. When it came to residual error, it went from 4335 to 1.041, which is another huge step in showing
there could be a significant relationship between the variables. The R-squared value shows that the first model went
from having less than a 1% variability of price due to the table and now with the transformed model that has gone up to
1.5%. Although this isn't a lot the transformation has increased the way these variables interact with each other. 

Looking at both of the models' graphs the change is there but not that significant. The biggest change can be seen in 
the standardized residuals plot where compared to the first model and the transformed model there is noticable change
in how the plotted points hug the dotted line at the top of the line where it tapered off from it in the first model. 
